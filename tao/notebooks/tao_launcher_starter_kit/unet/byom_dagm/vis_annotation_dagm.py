...
def visualize_masks(img_dir, mask_dir, vis_dir, num_imgs=None, num_classes=2):
    ...
    mask_ext = ".PNG"
    img_names = glob.glob(os.path.join(img_dir, f"*.PNG"))
    if not num_imgs:
        num_imgs = len(img_names)

    for img_path in img_names[:num_imgs]:
        orig_image = cv2.imread(img_path)
        output_height = orig_image.shape[0]
        output_width = orig_image.shape[1]
        
        img_name = os.path.basename(img_path).replace(mask_ext, "")

        segmented_img = np.zeros((output_height, output_width, 3))
        pred = cv2.imread(os.path.join(mask_dir, f"{img_name}_label{mask_ext}"), 0)
        ...
        cv2.imwrite(os.path.join(vis_dir, f"{img_name}.PNG"), fused_img)
